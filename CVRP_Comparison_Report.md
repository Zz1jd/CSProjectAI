...
### 问题描述
原始版本（基于 `funsearch_cvrp_original.ipynb` 及旧版 `implementation`）在解决 CVRP（带容量限制的车辆路径问题）时面临以下核心痛点：

The original version (based on `funsearch_cvrp_original.ipynb` and the legacy `implementation`) faced the following core issues when solving CVRP (Capacitated Vehicle Routing Problem):
...
| 指标 | 原始版本 (funsearch_cvrp_original) | 改进版本 (funsearch_cvrp_cot) | 提升/差异 |
| :--- | :--- | :--- | :--- |
...
| Metric | Original (funsearch_cvrp_original) | Improved (funsearch_cvrp_cot) | Gain / Difference |
| :--- | :--- | :--- | :--- |
